...
const Card = ({ id, name, image, category, price, onAddToCart }) => {
  ...
  return (
    ...
    <div className="w-full h-[500px]  flex flex-col items-center bg-[var(--cardColor)]   rounded-[15px] shadow-md overflow-hidden pb-2" key={id}>
      ...
      <button className="cursor-pointer px-7 py-3 lg:px-10 lg:py-3 bg-[var(--primary)] rounded-[10px] font-semibold uppercase text-[12px] lg:text-[15px]"
      onClick={()=>onAddToCart(product)}
      >{t("dataProducts.addtocard")}</button>
    </div>
  )
}
