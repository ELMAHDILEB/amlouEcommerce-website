...
const ShoppingCart = ({ onClosePopUp, cartItems = [], decreaseItem,incrementItem,removeItem, removeCart }) => {
    ...
    return (
        ...
        >
            <div className="sm:w-full md:w-[50%] h-full bg-[var(--colorBody)] flex flex-col gap-5">
                    ...
                    {cartItems.length === 0 ? (
                        ...
                        cartItems.map((item, index) => (
                            ...
                            >
                                ...
                                <h1 className="capitalize sm:text-sm md:text-xl">{item.name}</h1>
                                <span>x {item.quantity}</span>

                                <div className="w-fit flex flex-col items-center justify-center gap-2">
                                    <div className="w-full flex justify-between items-center">
                                        <button className="border-none outline-none cursor-pointer w-[20px] h-[20px] bg-[--SubColor] rounded-full flex items-center justify-center" onClick={()=>decreaseItem(item.id)}>
                                            -
                                        </button>
                                        <span>1</span>
                                        <button className="border-none outline-none cursor-pointer w-[20px] h-[20px] bg-[--SubColor] rounded-full flex items-center justify-center" onClick={()=>incrementItem(item.id)}>
                                            +
                                        </button>
                                    </div>

                                    <button className="capitalize text-[#ffaea9] underline text-[14px] cursor-pointer" onClick={() => removeItem(item.id)}>
                ...
                {/* Remove All Button */}
                {
                    cartItems.length === 0 ? "" : <button className="capitalize text-[#ffaea9] underline text-[14px] cursor-pointer" onClick={removeCart}>
                ...
                >
                    ...
                    <button className="w-[50%] py-7 bg-[var(--primary)] cursor-pointer">
