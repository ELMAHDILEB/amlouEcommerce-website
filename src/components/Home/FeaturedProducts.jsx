...
const FeaturedProducts = () => {
  ...
  {
    FeaProducts.map(({ imgSrc, title, price, link }, index) => {
      return (
        <article className="w-full  h-[350px] flex flex-col items-center justify-between bg-[var(--cardColor)] cursor-pointer rounded-[15px]" key={index * 4}>
          ...
          <div className="w-full h-[130px] flex flex-col items-center justify-center gap-2 text-center">
            ...
            <div className="flex items-center gap-2 py-4">
              <NavLink to={link} className="w-[40px] h-[40px] flex items-center justify-center bg-[var(--primary)] text-white text-[22px] border border-[var(--blackColor)] rounded-full cursor-pointer" role="button" tabIndex="1" aria-label="button">
