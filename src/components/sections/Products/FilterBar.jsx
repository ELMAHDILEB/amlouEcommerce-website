...
const FilterBar = ({ selectedCategory, setSelectedCategory, selectedSort, setSelectedSort, priceValue, setPriceValue }) => {
  ...
  const toggleCategoriesDrop = () => { setOpenCategoriesDrop(!openCategoriesDrop); setOpenSortDrop(false); }
  const toggleSortDrop = () => { setOpenSortDrop(!openSortDrop); setOpenCategoriesDrop(false); }
  ...
  return (
    <section className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 font-montserrat">
      ...
      <DropDown
        title={t("pageProducts.category.title")}
        subTitle={categories.find(cat => cat.value === selectedCategory)?.label}
        items={categories}
        selectedItem={selectedCategory}
        setSelectedItem={setSelectedCategory}
        isOpen={openCategoriesDrop}
        setIsOpen={toggleCategoriesDrop}
      />
